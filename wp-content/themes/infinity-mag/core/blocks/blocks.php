 ...
 */
add_action( 'dw_half_width_builder', 'dw_builder_half_width', 10, 3 );
function dw_builder_half_width( $place_number = 1, $before = '', $after = '' ) {
	if ( ! function_exists( 'fw_get_db_settings_option' ) ) {
		return;
	}
	$all_blocks = dw_get_setting( 'home_half_width_' . $place_number );
	if ( $all_blocks ) {
		echo $before;
