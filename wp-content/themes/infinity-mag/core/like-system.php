 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_window_process_like', 'window_mag_process_like' );
add_action( 'wp_ajax_window_process_like', 'window_mag_process_like' );
function window_mag_process_like() {
	// Security
	...
	if ( $post_id != '' ) {
		...
		if ( ! window_mag_already_liked( $post_id, $is_comment ) ) { // Like the post
			if ( is_user_logged_in() ) { // user is logged in
				$user_id    = get_current_user_id();
				$post_users = window_mag_post_user_likes( $user_id, $post_id, $is_comment );
				if ( $is_comment == 1 ) {
					// Update User & Comment
				...
				$user_ip    = window_mag_get_ip();
				$post_users = window_mag_post_ip_likes( $user_ip, $post_id, $is_comment );
				// Update Post
				if ( $post_users ) {
					if ( $is_comment == 1 ) {
			...
			$response['icon']   = window_mag_get_liked_icon();
			...
			$response['icon']   = window_mag_get_unliked_icon();
		}
		if ( $is_comment == 1 ) {
		...
		$response['count']   = window_mag_get_like_count( $like_count );
 ...
 * @since    0.5
 */
function window_mag_already_liked( $post_id, $is_comment ) {
	$post_users = null;
	$user_id    = null;
	if ( is_user_logged_in() ) { // user is logged in
		$user_id         = get_current_user_id();
...
} // window_already_liked()

/**
 * Output the like button
 * @since    0.5
 */
function window_mag_get_likes_button( $post_id, $is_comment = null ) {
	...
	$count      = window_mag_get_like_count( $like_count );
	$icon_empty = window_mag_get_unliked_icon();
	$icon_full  = window_mag_get_liked_icon();
 ...
 * @since    0.5
 */
function window_mag_post_user_likes( $user_id, $post_id, $is_comment ) {
	$post_users      = '';
	$post_meta_users = ( $is_comment == 1 ) ? get_comment_meta( $post_id, "_user_comment_liked" ) : get_post_meta( $post_id, "_user_liked" );
	if ( count( $post_meta_users ) != 0 ) {
		$post_users = $post_meta_users[0];
 ...
 * @since    0.5
 */
function window_mag_post_ip_likes( $user_ip, $post_id, $is_comment ) {
	$post_users      = '';
	$post_meta_users = ( $is_comment == 1 ) ? get_comment_meta( $post_id, "_user_comment_IP" ) : get_post_meta( $post_id, "_user_IP" );
 ...
 * @since    0.5
 */
function window_mag_get_liked_icon() {
 ...
 * @since    0.5
 */
function window_mag_get_unliked_icon() {
...
// User Profile List
add_action( 'show_user_profile', 'window_mag_show_user_likes' );
add_action( 'edit_user_profile', 'window_mag_show_user_likes' );
