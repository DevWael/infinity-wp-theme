 ...
 */
function window_mag_scrape_instagram( $username ) {
	$transient_name = 'window-mag-instagram-';
	...
	if ( false === ( $instagram = get_transient( $transient_name . sanitize_title_with_dashes( $username ) ) ) ) {
		...
		foreach ( $images as $image ) {
			// Note: keep hashtag support different until these JSON changes stabilise
			// these are mostly the same again now
			switch ( substr( $username, 0, 1 ) ) {
				case '#': // Hashtag
					if ( false === $image['node']['is_video'] ) {
						$type = 'video';
					} else {
						$type = 'image';
					}
				...
				default:
					...
					$instagram[] = array(
						'description' => $caption,
						'link'        => trailingslashit( '//instagram.com/p/' . $image['node']['shortcode'] ),
						'time'        => date( 'd-m-Y', $image['node']['taken_at_timestamp'] ),
						'comments'    => $image['node']['edge_media_to_comment']['count'],
						'likes'       => $image['node']['edge_media_preview_like']['count'],
						'thumbnail'   => preg_replace( '/^https?\:/i', '', $image['node']['thumbnail_resources'][0]['src'] ),
						'small'       => preg_replace( '/^https?\:/i', '', $image['node']['thumbnail_resources'][2]['src'] ),
...
/**
 * Display instagram content returned by window-mag_scrape_instagram()
 *
 * @param $username
 ...
 */
if ( ! function_exists( 'window_mag_instagram_photos_views' ) ) {
	function window_mag_instagram_photos_views( $username, $photos_count = 6, $before = '', $after = '' ) {
		if ( $username ) { // username must be available
			$instagram_media = window_mag_scrape_instagram( $username ); //photos array data from instagram
			if ( is_wp_error( $instagram_media ) ) {
				...
				foreach ( $instagram_media as $photo ) {
					...
					?>
                    <div class="window-instagram-photo instagram-item">
